...
#Boson词典情感分析计算情感得分
df_comment = pd.read_csv('data_clean.csv',usecols=[1])
comment = df_comment['comment'].tolist()
score = []
i = 0
for line in comment:
    i = i+1
    e = sentiment_score(line)
    print(str(i)+' '+line+'  '+str(e))
    score.append(e)
...
for line in comment:
    if i <= 500:
        ...
        try:
            ...
            print(str(i)+' '+line+'  '+str(s))
        except:
            score.append(0.5)
            print(str(i)+' '+'计算未成功')
    else:break
df_score = pd.DataFrame(score)
df_score.to_csv('score_snownlp.csv',index=False)

#对比两种情感分析模型准确度
df_comment = pd.read_csv('data_clean.csv',usecols=[1],nrows=1000)
df_score_boson = pd.read_csv('score.csv',nrows=1000)
df_score_snow = pd.read_csv('score_snownlp.csv',nrows=1000)
df = pd.concat([df_comment,df_score_boson],axis=1)
df_compare = pd.concat([df,df_score_snow],axis=1)
